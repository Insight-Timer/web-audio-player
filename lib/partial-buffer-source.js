...
function createPartialBufferSource (src, opt) {
  ...
  emitter.play = function () {
    ...
    if (loop) {
      ...
      if (typeof opt.loopStart === 'number') {
        bufferNode.loopStart = opt.loopStart
      }
      if (typeof opt.loopEnd === 'number') {
        bufferNode.loopEnd = opt.loopEnd
      }
    }
  ...
  function startLoad (src) {
    downloadManager = new PartialXhrAudio(
      audioContext,
      src,
      function audioDecoded (err, decoded, chunkBytes, totalBytes) {
        if (err) return emitter.emit('error', err)

        // Since we're using chunked bytes, we can't actually get the exact duration until have the full chunks/file.
        // even using ID3 also not reliable because not all files have ID3 and Length set in ID3 metadata.
        // So here if we get the full file, use that duration otherwise we use whichever duration is longer between estimated and decoded duration.
        if (!duration && chunkBytes !== totalBytes) {
    ...
    downloadManager
      .setDownloadLength(
        opt.bufferSize || PartialXhrAudio.DEFAULT_DOWNLOAD_LENGTH
      )
      .start()
  }
