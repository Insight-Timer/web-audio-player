...
function PartialXhrAudio(audioContext, src, cb, progress, decoding) {
    ...
    var totalLength = 0
    var maxDownloadLength = 5 * 1000000 // approx 5mb per download

    // expose methods and variables
    return {
        setDownloadLength(length) {
    ...
    function start(recursive) {
        ...
        var xhrObject = xhr(opts, function (err, resp, newBuffer) {
            ...
            if (!resp.headers['content-range']) return cb(new Error('`content-range` not found in response headers'))
            if (!totalLength) {
                totalLength = parseInt(resp.headers['content-range'].split('/')[1])
            }

            downloadedLength += parseInt(resp.headers['content-length'])
            downloadedBuffer = appendBuffer(downloadedBuffer, newBuffer)
            decode()

            // start the next download in 2 sec.
            if (recursive === true) {
                setTimeout(function () {
                    start(recursive)
                }, 2000)
    ...
    function getRangeHeader() {
        ...
        if ((downloadedLength === 0 && totalLength === 0) || willDownloadedlength < totalLength)
    ...
    function decode() {
        ...
        audioContext.decodeAudioData(preDecodedBuff || downloadedBuffer, function (decoded) {
